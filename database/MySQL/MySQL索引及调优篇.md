...
![img](MySQL索引及调优篇.assets/image-20220616152727234.png)
...
![img](MySQL索引及调优篇.assets/image-20220617160325201.png)

![image-20220617160413479](MySQL索引及调优篇.assets/image-20220617160413479.png)

![img](MySQL索引及调优篇.assets/image-20220617160533122.png)

![image-20220617160625006](MySQL索引及调优篇.assets/image-20220617160625006.png)

![img](MySQL索引及调优篇.assets/image-20220617160813548.png)
...
![img](MySQL索引及调优篇.assets/image-20220617161126022.png)
...
![img](MySQL索引及调优篇.assets/image-20220617161635521.png)

这里都懒得说了。

### 6.2 Hash查询

![img](MySQL索引及调优篇.assets/image-20220617161946230.png)
...
![img](MySQL索引及调优篇.assets/image-20220617162548697.png)
...
![img](MySQL索引及调优篇.assets/image-20220617163202156.png)
...
![img](MySQL索引及调优篇.assets/image-20220617163619721.png)
...
![img](MySQL索引及调优篇.assets/image-20220617163952166.png)
...
![img](MySQL索引及调优篇.assets/image-20220617165045803.png)
...
![img](MySQL索引及调优篇.assets/image-20220617170124200.png)
...
![img](MySQL索引及调优篇.assets/image-20220617170454023.png)
...
![img](MySQL索引及调优篇.assets/image-20220617170628394.png)
...
![img](MySQL索引及调优篇.assets/image-20220617171011102.png)
...
![img](MySQL索引及调优篇.assets/image-20220617171331282.png)

![img](MySQL索引及调优篇.assets/image-20220617171434206.png)

> B 树和 B+ 树都可以作为索引的数据结构，在 MySQL 中采用的是 B+ 树。 但B树和B+树各有自己的应用场景，不能说B+树完全比B树好，反之亦然。

**思考题：为了减少IO，索引树会一次性加载吗？**

![img](MySQL索引及调优篇.assets/image-20220617171614460.png)

**思考题：B+树的存储能力如何？为何说一般查找行记录，最多只需1~3次磁盘IO**

![img](MySQL索引及调优篇.assets/image-20220617172426725.png)

**思考题：为什么说B+树比B-树更适合实际应用中操作系统的文件索引和数据库索引？**

![img](MySQL索引及调优篇.assets/image-20220617175142810.png)

**思考题：Hash 索引与 B+ 树索引的区别**

![img](MySQL索引及调优篇.assets/image-20220617175230327.png)

**思考题：Hash 索引与 B+ 树索引是在建索引的时候手动指定的吗？**

![img](MySQL索引及调优篇.assets/image-20220617175309115.png)
...
![img](MySQL索引及调优篇.assets/image-20220617175440527.png)
...
![img](MySQL索引及调优篇.assets/image-20220706164201695.png)

![img](MySQL索引及调优篇.assets/image-20220706164359539.png)
...
![img](MySQL索引及调优篇.assets/image-20220706165231022.png)
...
![img](MySQL索引及调优篇.assets/image-20220707124343085.png)

## 3. 反范式化

### 3.1 概述

![img](MySQL索引及调优篇.assets/image-20220707124741675.png)
...
![img](MySQL索引及调优篇.assets/image-20220707125545891.png)
...
![img](MySQL索引及调优篇.assets/image-20220707125642908.png)
...
![img](MySQL索引及调优篇.assets/image-20220707125718469.png)
...
![img](MySQL索引及调优篇.assets/image-20220707131428597.png)
...
![img](MySQL索引及调优篇.assets/image-20220707163807205.png)

进货单表：

![img](MySQL索引及调优篇.assets/image-20220707163828614.png)
...
![img](MySQL索引及调优篇.assets/image-20220707165011050.png)

进货单头表：

![img](MySQL索引及调优篇.assets/image-20220707165038108.png)

这2个表都满足第三范式的要求了。

### 7.4 反范式化：业务优先的原则

![img](MySQL索引及调优篇.assets/image-20220707165459547.png)
...
![img](MySQL索引及调优篇.assets/image-20220707164146216.png)

商品信息表：

![img](MySQL索引及调优篇.assets/image-20220707164227845.png)

这样一来，我们就避免了冗余数据，而且还能够满足业务的需求，这样的数据库设计，才是合格的设计。

## 8. ER模型

![img](MySQL索引及调优篇.assets/image-20220707170027637.png)
...
![img](MySQL索引及调优篇.assets/image-20220707171335255.png)

![img](MySQL索引及调优篇.assets/image-20220707171412363.png)

![img](MySQL索引及调优篇.assets/image-20220707171915637.png)

**下面我们再把弱实体转换成数据表：**

![img](MySQL索引及调优篇.assets/image-20220707172033399.png)

![img](MySQL索引及调优篇.assets/image-20220707172052236.png)

![img](MySQL索引及调优篇.assets/image-20220707172143793.png)

![img](MySQL索引及调优篇.assets/image-20220707172217772.png)

![img](MySQL索引及调优篇.assets/image-20220707172236606.png)

![img](MySQL索引及调优篇.assets/image-20220707172259143.png)

#### 2. 一个多对多的关系转换成一个数据表

![img](MySQL索引及调优篇.assets/image-20220707172350226.png)

#### 3. 通过外键来表达1对多的关系

![img](MySQL索引及调优篇.assets/image-20220707172609833.png)

#### 4. 把属性转换成表的字段

![img](MySQL索引及调优篇.assets/image-20220707172819174.png)
...
![img](MySQL索引及调优篇.assets/image-20220707173028203.png)

**2. 数据表中的字段个数越少越好**

![img](MySQL索引及调优篇.assets/image-20220707173402491.png)

**3. 数据表中联合主键的字段个数越少越好**

![img](MySQL索引及调优篇.assets/image-20220707173522971.png)

**4. 使用主键和外键越多越好**

![img](MySQL索引及调优篇.assets/image-20220707173557568.png)
...
![img](MySQL索引及调优篇.assets/image-20220707175250944.png)
...
![img](MySQL索引及调优篇.assets/image-20220707175805226.png)

在此上图说明name和code的起名方法

![img](MySQL索引及调优篇.assets/image-20220707175827417.png)
...
![img](MySQL索引及调优篇.assets/image-20220707180327712.png)

创建好主页面如图所示，但是右边的按钮和概念模型略有差别，物理模型最常用的三个是 `table(表)` ， `view(视图)`， `reference(关系) `；

![img](MySQL索引及调优篇.assets/image-20220707180418090.png)

鼠标先点击右边table这个按钮然后在新建的物理模型点一下，即可新建一个表，然后双击新建如下图所示，在General的name和code填上自己需要的，点击应用即可），如下图：

![img](MySQL索引及调优篇.assets/image-20220707180449212.png)

然后点击Columns,如下图设置，非常简单，需要注意的就是P（primary主键） , F （foreign key外键） , M（mandatory强制性的，代表不可为空） 这三个。

![img](MySQL索引及调优篇.assets/image-20220707180537251.png)

在此设置学号的自增（MYSQL里面的自增是这个AUTO_INCREMENT），班级编号同理，不多赘述！

![img](MySQL索引及调优篇.assets/image-20220707180556645.png)

在下面的这个点上对号即可，就设置好了自增

![img](MySQL索引及调优篇.assets/image-20220707180619440.png)

全部完成后如下图所示。

![img](MySQL索引及调优篇.assets/image-20220707180643107.png)

班级物理模型同理如下图所示创建即可

![img](MySQL索引及调优篇.assets/image-20220707180723698.png)

![img](MySQL索引及调优篇.assets/image-20220707180744600.png)

完成后如下图所示

![img](MySQL索引及调优篇.assets/image-20220707180806150.png)
...
![img](MySQL索引及调优篇.assets/image-20220707180828164.png)

做完上面的操作，就可以双击中间的一条线，显示如下图，修改name和code即可

![img](MySQL索引及调优篇.assets/image-20220707183743297.png)

但是需要注意的是，修改完毕后显示的结果却如下图所示，并没有办法直接像概念模型那样，修改过后 显示在中间的那条线上面，自己明白即可。

![img](MySQL索引及调优篇.assets/image-20220707193816176.png)

学习了多对一或者一对多的关系，接下来学习多对对的关系，同理自己建好老师表，这里不在叙述，记得老师编号自增，建好如下图所示

![img](MySQL索引及调优篇.assets/image-20220707193932694.png)

下面是多对多关系的关键，由于物理模型多对多的关系需要一个中间表来连接，如下图，只设置一个字 段，主键，自增

![img](MySQL索引及调优篇.assets/image-20220707193957629.png)

点击应用，然后设置Columns，只添加一个字段

![img](MySQL索引及调优篇.assets/image-20220707194048843.png)

这是设置字段递增，前面已经叙述过好几次

![img](MySQL索引及调优篇.assets/image-20220707194111885.png)

设置好后如下图所示，需要注意的是有箭头的一方是一，无箭头的一方是多，即一对多的多对一的关系 需要搞清楚，学生也可以有很多老师，老师也可以有很多学生，所以学生和老师都可以是主体；

![img](MySQL索引及调优篇.assets/image-20220707194138137.png)

可以看到添加关系以后学生和教师的关系表前后发生的变化

![img](MySQL索引及调优篇.assets/image-20220707194158936.png)
