...
![](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-08-11/04980fab-0d2c-4d91-b54c-58055c2decc0.jpg)
...
![](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-08-11/5d089ada-1e40-4760-8852-56f524e29d8c.jpg)
...
![jvm-pc-counter](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-11/7420f018-bbcf-4c53-8959-8c807eed2674_0082zybply1gc5kmznm1sj31m50u0wph.jpg)

（分析：进入class文件所在目录，执行 `javap -v xx.class` 反解析（或者通过 IDEA 插件 `Jclasslib` 直接查看，上图），可以看到当前类对应的Code区（汇编指令）、本地变量表、异常表和代码行偏移量映射表、常量池等信息。）

关于类的字节码相关可以看：[JVM基础 - 类字节码详解](java/JVM/类字节码详解)
...
![img](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-11/21b694be-f5d5-4b4b-8d2a-91c278afb965_0082zybply1gc9lezaxrbj319v0u0k4w.jpg)
...
![jvm-stack-frame](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-11/b6d4e712-258f-4493-8cd0-771e29c1120f_0082zybply1gc8tjehg8bj318m0lbtbu.jpg)
...
- 如果当前帧是由**构造方法**或**实例方法**创建的，那么该对象引用 this 将会存放在 index 为 0 的 Slot 处，其余的参数按照参数表顺序继续排列（这里就引出一个问题：静态方法中为什么不可以引用 this，就是因为this 变量不存在于当前方法的局部变量表中）
...
![img](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-11/5a2438e8-b45d-474d-8a6b-478744dbaa16_0082zybply1gc9s12g5wlj31li0owdm9.jpg)
...
- 每一个操作数栈都会拥有一个明确的栈深度用于存储数值，其所需的最大深度在编译期就定义好了，保存在方法的 Code 属性的 `max_stack` 数据项中，如下图示：

  ![操作数栈最大深度](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-22/f4fa1f50-1247-47bf-85aa-dfb4582ca7d4_操作数栈最大深度.png)
...
- 操作数栈中元素的数据类型必须与字节码指令的序列严格匹配，这由编译器在编译期间进行验证，同时在类加载过程中的类检验阶段的数据流分析阶段要再次验证，详情查看[类加载过程中的验证](/java/JVM/Java类加载机制?id=连接)
...
- **每一个栈帧内部都包含一个指向[运行时常量池](java/JVM/JVM内存结构?id=_54-运行时常量池)中该栈帧所属方法的引用**。包含这个引用的目的就是为了支持当前方法的代码能够实现动态链接(Dynamic Linking)。
...
![jvm-dynamic-linking](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-11/19283432-b158-44f0-8a02-017b51029523_0082zybply1gca4k4gndgj31d20o2td0.jpg)
...
![JDK7](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-11/d3913c18-ffeb-46a3-895b-135856de9c97_00831rSTly1gdbr7ek6pfj30ci0560t4.jpg)
...
![img](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-11/14cb49ad-a713-4ff4-8b0f-3dc75f947394_007S8ZIlly1gg06065oa9j31kw0u0q69.jpg)
...
#### 什么是 TLAB （Thread Local Allocation Buffer  线程本地分配缓冲区）?
...
- **方法区（method area）**只是 JVM 规范中定义的一个概念，用于`存储类信息`、`常量池`、`静态变量`、`JIT编译后的代码`等数据，并没有规定如何去实现它，不同的厂商有不同的实现。而**永久代（PermGen）**是Hotspot 虚拟机特有的概念， Java8 的时候又被**元空间**取代了，永久代和元空间都可以理解为方法区的落地实现。
...
![img](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-22/0bb06ff8-c046-4904-8303-49a58dc9a68c_栈、堆、方法区的交互关系.png)
...
![img](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-11/f7934c79-aff2-4730-8a92-c8357b818a15_007S8ZIlly1gg9i91ze2gj320i0riahe.jpg)
...
![img](https://vue-admin-imgages.oss-cn-hangzhou.aliyuncs.com/2022-09-11/a1f8cf3f-d89d-4b7f-8318-1bab7685e3b1_007S8ZIlly1gg04ve34c2j30z00u0dp7.jpg)
